...
import { Activity, AlertTriangle, Check, CheckCircle2, ChevronRight, CircleStop, Copy, Database, Globe2, Loader2, LockKeyhole, Network, Play, RefreshCw, RotateCcw, Save, Settings2, ShieldCheck, Terminal, Trash2, Wifi, X, Zap } from 'lucide-react'
...
if (tokenFromUrl) {
  window.localStorage.setItem('dns-switcher-session-token', tokenFromUrl)
  const cleanUrl = `${window.location.pathname}${window.location.hash}`
  window.history.replaceState({}, document.title, cleanUrl)
}
const apiToken = tokenFromUrl ?? window.localStorage.getItem('dns-switcher-session-token') ?? 'development-only-token'
const api = async <T,>(path: string, init: RequestInit = {}): Promise<T> => {
  const headers = new Headers(init.headers)
  headers.set('X-Session-Token', apiToken)
  if (init.body) headers.set('Content-Type', 'application/json')
  const response = await fetch(path, { ...init, headers })
  if (!response.ok) { const body = await response.json().catch(() => ({})); throw new Error(body.detail ?? `Errore HTTP ${response.status}`) }
...
export default function App() {
  ...
  const load = useCallback(async () => {
    ...
    } catch (error) { setNotice({ kind: 'error', text: error instanceof Error ? error.message : 'Backend non disponibile' }) }
  }, [])
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(`${protocol}://${host}/ws/terminal?token=${encodeURIComponent(apiToken)}`)
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand"><img className="brand-mark" src="/icon.svg" alt=""/><div><div className="brand-name">DNS Switcher <span>Pro</span></div><div className="eyebrow">LOCAL NETWORK CONTROL · v1.1.3</div></div></div><div className="top-actions"><div className="connection-pill"><span className="pulse-dot"/> Backend locale</div><button className={`settings-button ${view === 'settings' ? 'is-active' : ''}`} aria-label={view === 'dashboard' ? 'Apri impostazioni' : 'Torna alla dashboard'} onClick={() => setView(view === 'dashboard' ? 'settings' : 'dashboard')}><Settings2 size={17}/><span>{view === 'dashboard' ? 'Impostazioni' : 'Dashboard'}</span></button></div></header>
    <main className="content">
      ...
      </motion.div> : <motion.div key="settings" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}><SettingsView form={form} setForm={setForm} credentials={credentials} credentialForm={credentialForm} setCredentialForm={setCredentialForm} saving={saving} testing={testing} onSave={saveSettings} onSaveCredentials={saveCredentials} onTest={testConnection} onReset={async () => { const next = await api<Settings>('/api/settings/reset', { method: 'POST' }); setForm(next); setSettings(next); setNotice({ kind: 'success', text: 'Valori predefiniti ripristinati' }) }} onBack={() => setView('dashboard')} /></motion.div>}</AnimatePresence>
    </main><footer className="app-footer"><span>Copyright 2026 Alex Lignola · Created by Alex Lignola</span><span className="footer-secure"><LockKeyhole size={13}/> Nessun dato lascia questo PC</span></footer>
  </div>
}
